...
MPU6050::MPU6050(int8_t addr, bool run_update_thread) {
	...
	f_dev = open("/dev/i2c-4", O_RDWR); //Open the I2C device file
	if (f_dev < 0) { //Catch errors
		std::cout << "ERR (MPU6050.cpp:MPU6050()): Failed to open /dev/i2c-4. Please check that I2C is enabled with raspi-config\n"; //Print error message
	}
